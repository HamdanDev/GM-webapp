                <div class="section d-none" id="paimnt">
                    <div class="row g-3 mb-3">
                        <div class="col-6 col-lg-3">
                            <div class="card-ca h-100">
                                <div class="icon"><i class="bi bi-cash-stack"></i></div>
                                <div class="content">
                                    <p class="title fw-bold">Total Paiements</p>
                                    <h2 class="fw-bold">10 500 MAD</h2>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-lg-3">
                            <div class="card-ca h-100">
                                <div class="icon"><i class="bi bi-check-circle"></i></div>
                                <div class="content">
                                    <p class="title fw-bold">Réussis</p>
                                    <h2 class="fw-bold">20</h2>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-lg-3">
                            <div class="card-ca h-100">
                                <div class="icon"><i class="bi bi-hourglass-split"></i></div>
                                <div class="content">
                                    <p class="title fw-bold">En attente</p>
                                    <h2 class="fw-bold">12</h2>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-lg-3">
                            <div class="card-ca h-100">
                                <div class="icon"><i class="bi bi-arrow-counterclockwise"></i></div>
                                <div class="content">
                                    <p class="title fw-bold">Remboursements</p>
                                    <h2 class="fw-bold">2</h2>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="products-page">
                        <div class="stock-header">
                            <div>
                                <h2>Paiements</h2>
                                <p class="subtitle">Visualisez vos revenus et paiements reçus.</p>
                            </div>
                            <div class="header-actions">
                                <button class="export-btn"><i class="bi bi-download me-2"></i>Exporter</button>
                            </div>
                        </div>
                        <div class="top-bar">
                            <div class="search-box">
                                <i class="bi bi-search"></i>
                                <input placeholder="Rechercher un paiement..." type="text" />
                            </div>
                            <div class="filter-box">
                                <i class="bi bi-funnel"></i>
                                <select>
                                    <option>Tous statuts</option>
                                    <option>Payé</option>
                                    <option>En attente</option>
                                    <option>Échoué</option>
                                </select>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="product-table">
                                <thead>
                                    <tr>
                                        <th>ID Paiement</th>
                                        <th>ID Commande</th>
                                        <th>Client</th>
                                        <th>Montant</th>
                                        <th>Méthode</th>
                                        <th>Statut</th>
                                        <th>Date</th>
                                        <th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>#PAY0188</td>
                                        <td>#CMD1524</td>
                                        <td>
                                            <h6 class="mb-0">Sara Sarati</h6><small>#C-102</small>
                                        </td>
                                        <td>250 MAD</td>
                                        <td>Visa</td>
                                        <td><span class="stock ok">Payé</span></td>
                                        <td>15/05/2026</td>
                                        <td>
                                            <div class="action-buttons">
                                                <button class="icon-btn view-btn" title="Voir détails"><i class="bi bi-eye"></i></button>
                                                <button class="icon-btn" title="Télécharger facture"><i class="bi bi-download"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>#PAY0189</td>
                                        <td>#CMD1525</td>
                                        <td>
                                            <h6 class="mb-0">Sara Sarati</h6><small>#C-103</small>
                                        </td>
                                        <td>400 MAD</td>
                                        <td>Visa</td>
                                        <td><span class="stock ok">Payé</span></td>
                                        <td>16/05/2026</td>
                                        <td>
                                            <div class="action-buttons">
                                                <button class="icon-btn view-btn" title="Voir détails"><i class="bi bi-eye"></i></button>
                                                <button class="icon-btn" title="Télécharger facture"><i class="bi bi-download"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>#PAY0190</td>
                                        <td>#CMD1526</td>
                                        <td>
                                            <h6 class="mb-0">Sara Sarati</h6><small>#C-104</small>
                                        </td>
                                        <td>150 MAD</td>
                                        <td>Cash</td>
                                        <td><span class="stock low">En attente</span></td>
                                        <td>17/05/2026</td>
                                        <td>
                                            <div class="action-buttons">
                                                <button class="icon-btn view-btn" title="Voir détails"><i class="bi bi-eye"></i></button>
                                                <button class="icon-btn" title="Confirmer"><i class="bi bi-check-circle"></i></button>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        <div class="custom-pagination">
                            <button class="pagination-arrow">«</button>
                            <button class="pagination-number active">1</button>
                            <button class="pagination-number">2</button>
                            <button class="pagination-number">3</button>
                            <button class="pagination-arrow">»</button>
                        </div>
                    </div>
                </div>
                <!-- AVIS -->
